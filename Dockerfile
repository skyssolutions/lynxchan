...
RUN echo "+ Starting Imagemagic download and build" && \
    cd /usr/local/src && \
    wget https://ftp.acc.umu.se/mirror/imagemagick.org/ftp/ImageMagick-6.9.12-61.tar.gz && \
    tar -xvf ImageMagick-6.9.12-61.tar.gz && cd ImageMagick-6.9.12-61 && \
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y zlib1g-dev libncurses5-dev libgdbm-dev libnss3-dev libssl-dev libsqlite3-dev libreadline-dev libffi-dev curl libbz2-dev && \
    cd /usr/local/src && wget https://www.python.org/ftp/python/3.10.6/Python-3.10.6.tar.xz && tar -xvf Python-3.10.6.tar.xz && cd Python-3.10.6 && \
